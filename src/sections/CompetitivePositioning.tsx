...
function ViewTabs({ tab, setTab }: { tab: Tab; setTab: (t: Tab) => void }) {
  return <SectionTabs tabs={TABS.map((t) => ({ id: t, label: t }))} active={tab} onSelect={(id) => setTab(id as Tab)} />
}

function IfrsNote({ boxed }: { boxed?: boolean }) {
  return (
    <p className={`flex items-start gap-1.5 ${boxed ? 'rounded-lg border border-[#CDE7E4] bg-[#F1FAF8] px-3 py-2' : 'border-b border-soft-border bg-[#F1FAF8] px-3.5 py-2'} text-[11px] leading-snug text-[#0E6F6D]`}>
  ...
  return (
    <div ref={focusRef} className={`space-y-4 ${arrived ? 'insight-arrival rounded-2xl' : ''}`}>
      ...
      {tab === 'Table' ? (
        ...
        <>
          {/* Same control language as the table view — tabs on the left, the
              accounting-basis lens on the right — but standalone above the
              scorecard grid (which is itself a set of cards). */}
          <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-2">
            <ViewTabs tab={tab} setTab={setTab} />
            <AccountingBasisToggle value={basis} onChange={setBasis} compact />
          </div>
          {basis === 'ifrs' && <IfrsNote boxed />}
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-[72fr_28fr]">
            {/* HERO scorecard — a soft navy→teal gradient header carrying the focal
                company's overall standing and the four sub-scores, over the peer
                heatmap. */}
